...
HEIGHT = 60
WIDTH  = 160
gammas_i_h = [ALPHA_H + (i%160) * (THETA_H / WIDTH) for i in range(0,9600)]
gammas_i_v = [ALPHA_V + int(i/160) * (THETA_V / HEIGHT) for i in range(0,9600)]
sins_h = [0.001 * math.sin(gammas_i_h[i]) for i in range(0,9600)]
coss_v = [math.cos(gammas_i_v[i]) for i in range(0,9600)]
tans_h = [1/math.tan(gammas_i_h[i]) for i in range(0,9600)]
tans_v = [-math.tan(gammas_i_v[i]) for i in range(0,9600)]
cossins = [sins_h[i]*coss_v[i] for i in range(0,9600)]
_PointMasked9600Amp = 9600 * _PointDistanceAmplitudeMasked
#FIN
...
class FrameBuilder:
    ...
    def composeDistanceAmplitudeFrame(self, dataArray) :
        ...
        _points = _PointMasked9600Amp.from_buffer(dataArray)

        #i_data_point = 0
        for i_data_point, _p in enumerate(_points):
            #OPTIMIZACIONES PROPIAS
            #x = i_data_point % 160
            ...
            if(
                ...
                if (
                    ...
                    #Z = abs(Z*coss_v[i_data_point])
                    Z = abs(distance*cossins[i_data_point])
                    X = Z*tans_h[i_data_point]
                    Y = Z*tans_v[i_data_point]
